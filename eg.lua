...
if not userKey then
    LocalPlayer:Kick("[XZILER HUB]: No license provided! Please enter your key.")
    return
end
...
if not decodeSuccess or type(data) ~= "table" or not data.success then
    ...
    -- แก้ไขตรงนี้จากเครื่องหมาย + เป็น .. (จุดสองจุด) เพื่อต่อข้อความ String
...
if not runSuccess then
    -- แก้ไขตรงนี้จากเครื่องหมาย + เป็น .. เช่นกัน
